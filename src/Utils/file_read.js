...
async function postreadQuery(filename, startLine, endLine, payload, queryIndex, resp) {
...
async function putreadQuery(filename, startLine, endLine, payload, queryIndex, resp) {
    const fileStream = fs.createReadStream(filename);
    let currentLine = 0;
    let lines = [];
    const rl = readline.createInterface({
                ...
                if (error) {
                    ...
                    if (result.length > 1) {
                        if (result[0].command == 'INSERT' && result[0].rowCount >= 1) {
                            resolve({
                                'Message': 'Success',
                                'Row count': result[0].rowCount + ' rows inserted successfully.'
                            });
                        } else if (result[1].command == 'SELECT' && result[1].rowCount >= 1) {
                            resolve({
