...
const { selectQuery, insertQuery, updateQuery, executeSelectQuery, deleteQuery } = require("../../Utils/file_read");
const calibrationRouter = express.Router();
const { v4: uuidv4 } = require("uuid");
const { sendEmail, executeMailHistoryQuery, generateOrderTable, extractNameFromEmail, generateOrderEmailContent, insertOrders, extractNameInitials } = require("../../Services/calibration.service");

// Generate card number
calibrationRouter.post('/generate-card-number', varifyToken, tryCatch(async (req, resp) => {
...
// Frequency list
calibrationRouter.get('/frequency', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
...
// Purchase orders
calibrationRouter.post('/purchase-order', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            queryPath[45].PD_PRODUCT_INSTRUMENTCALIBRATIONSCHEDULE,
...
calibrationRouter.post('/instrument-schedule-registration', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        if (req.body.createdby == undefined) {
            ...
            throw new AppError(NOT_FOUND, 'Barcode or qrcode information not found', 404);
            ...
            properties.parse(
                ...
                function (error, data) {
                    ...
                    query = query.replace(/{instrument_id}/g, req.body.instrument_id.trim());
                    query = query.replace(/{cardnumber}/g, req.body.cardnumber.trim());
                    query = query.replace(/{measuringrange}/g, req.body.measuringrange.trim());
                    ...
                    query = query.replace(/{barcodeinformation}/g, req.body.barcodeinformation.trim());
                    query = query.replace(/{purchaseorder_id}/g, req.body.purchaseorder_id.trim());
                    query = query.replace(/{manufacturer}/g, req.body.manufacturer.trim());
...
calibrationRouter.post('/certificate-reference', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        if (req.body.id == undefined) {
            ...
            properties.parse(
                ...
                function (error, data) {
                    ...
                    query = query.replace(/{certificate_mdocid}/g, req.body.certificate_mdocid);
...
// Current day records
calibrationRouter.get('/currentday-status', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            queryPath[45].PD_PRODUCT_INSTRUMENTCALIBRATIONSCHEDULE,
...
// Calibration status
calibrationRouter.get('/calibration-status', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            queryPath[45].PD_PRODUCT_INSTRUMENTCALIBRATIONSCHEDULE,
            ...
            function (error, data) {
                ...
                executeSelectQuery(data.calibrationStatus).then((result) => {
                    if (req.query.searchString == '') {
                        if (req.query.range == 0) {
                            const filtereddata = result.slice(0, 50);
                            resp.send({ 'result': filtereddata });
                        } else {
                            const start = parseInt(req.query.range) + 1, end = parseInt(req.query.range) + 51;
                            const filtereddata = result.slice(start, end);
                            resp.send({ 'result': filtereddata });
                        }
                    } else {
                        const filtereddata = result.filter(item =>
                            item.instrumentname.toLowerCase().includes(req.query.searchString.toLowerCase())
                        );
                        resp.send({ 'result': filtereddata });
                    }
                }).catch((error) => {
...
calibrationRouter.post('/email-addresses', tryCatch(async (req, resp) => {
...
// Send mail
calibrationRouter.post('/send-instrument-order', varifyToken, tryCatch(async (req, resp) => {
    ...
    if (user) {
        ...
        const mailcontent = generateOrderEmailContent({ from: from, to: `${fromRec}-${toRec}`, table: table });
        const response =
            await sendEmail({
                from: req.body.from,
                to: req.body.to,
        ...
        if (response.success == true) {
            executeMailHistoryQuery({
                ...
                if (result.length > 0) {
                    insertOrders({ content: req.body.content, mailId: result[0]['id'] }).then(finalResult => {
...
calibrationRouter.post('/measuring-range-list', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            queryPath[45].PD_PRODUCT_INSTRUMENTCALIBRATIONSCHEDULE,
...
calibrationRouter.post('/one-instrument-from-product-table', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
...
calibrationRouter.get('/all-instrument-orders', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
...
calibrationRouter.post('/calibration-history-registration', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            ...
            function (error, data) {
                ...
                query = query.replace(/{instrumentcalibrationschedule_id}/g, req.body.instrumentcalibrationschedule_id);
...
calibrationRouter.put('/send-instrument-for-calibration', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            queryPath[45].PD_PRODUCT_INSTRUMENTCALIBRATIONSCHEDULE,
...
calibrationRouter.get('/outward-instruments', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            queryPath[48].PD_PRODUCT_INSTRUMENTCALIBRATION_HISTORY,
...
calibrationRouter.post('/instrument-outsource-workorder', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            ...
            function (error, data) {
                ...
                query = query.replace(/{outwardchallan_no}/g, req.body.outwardchallan_no);
                query = query.replace(/{outsource_date}/g, req.body.outsource_date);
                query = query.replace(/{subcontractor_id}/g, req.body.subcontractor_id);
...
calibrationRouter.put('/challan-reference-to-instruments', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            queryPath[48].PD_PRODUCT_INSTRUMENTCALIBRATION_HISTORY,
            ...
            function (error, data) {
                ...
                query = query.replace(/{id}/g, req.body.id);
                query = query.replace(/{outsourceworkorder_id}/g, req.body.outsourceworkorder_id);
                updateQuery(query, resp);
            });
    }
}));
...
calibrationRouter.get('/inward-instruments', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            queryPath[48].PD_PRODUCT_INSTRUMENTCALIBRATION_HISTORY,
...
calibrationRouter.put('/inward-spacific-instrument', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            queryPath[45].PD_PRODUCT_INSTRUMENTCALIBRATIONSCHEDULE,
            ...
            function (error, data) {
                ...
                query = query.replace(/{duedate}/g, req.body.duedate);
                query = query.replace(/{frequency}/g, req.body.frequency);
                query = query.replace(/{certificate_mdocid}/g, req.body.certificateid);
                query = query.replace(/{id}/g, req.body.id);
                updateQuery(query, resp);
            });
    }
...
// All challans list
calibrationRouter.get('/instruments-outsource-all-workorders', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            queryPath[48].PD_PRODUCT_INSTRUMENTCALIBRATION_HISTORY,
...
// One challan data
calibrationRouter.post('/one-challan-data', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            queryPath[48].PD_PRODUCT_INSTRUMENTCALIBRATION_HISTORY,
...
calibrationRouter.post('/spacific-instrument-card-numbers', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            queryPath[45].PD_PRODUCT_INSTRUMENTCALIBRATIONSCHEDULE,
...
calibrationRouter.post('/searched-instruments-data', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            queryPath[45].PD_PRODUCT_INSTRUMENTCALIBRATIONSCHEDULE,
...
calibrationRouter.post('/one-instrument-calibration-history', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            queryPath[48].PD_PRODUCT_INSTRUMENTCALIBRATION_HISTORY,
...
// Reject instrument
calibrationRouter.put('/reject-instrument', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            queryPath[45].PD_PRODUCT_INSTRUMENTCALIBRATIONSCHEDULE,
...
calibrationRouter.post('/add-rejected-instrument-to-history', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            queryPath[48].PD_PRODUCT_INSTRUMENTCALIBRATION_HISTORY,
...
calibrationRouter.get('/instrument-rejection-reasons', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            queryPath[45].PD_PRODUCT_INSTRUMENTCALIBRATIONSCHEDULE,
...
calibrationRouter.get('/rejected-instruments-data-for-new-instrument-order', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            queryPath[48].PD_PRODUCT_INSTRUMENTCALIBRATION_HISTORY,
...
// One instrumentOrder
calibrationRouter.get('/one-instrument-order', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
...
// Stored instruments
calibrationRouter.get('/stored-instruments', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            queryPath[48].PD_PRODUCT_INSTRUMENTCALIBRATION_HISTORY,
...
// Cancel calibration
calibrationRouter.post('/cancel-calibration', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            queryPath[45].PD_PRODUCT_INSTRUMENTCALIBRATIONSCHEDULE,
            ...
            function (error, data) {
                ...
                query = query.replace(/{startdate}/g, req.body.startdate);
                query = query.replace(/{duedate}/g, req.body.duedate);
                query = query.replace(/{id}/g, req.body.instrumentcalibrationscheduleId);
                query = query.replace(/{frequency}/g, req.body.frequency);
                query = query.replace(/{certificate_mdocid}/g, req.body.certificate_mdocid);
...
calibrationRouter.delete('/restore-stored-instruments', varifyToken, tryCatch(async (req, resp) => {
    const userData = authorizeToken(req.token);
    if (userData) {
        properties.parse(
            queryPath[48].PD_PRODUCT_INSTRUMENTCALIBRATION_HISTORY,
...
// Rejected instruments data
calibrationRouter.get('/rejected-instruments-data',
    varifyToken,
